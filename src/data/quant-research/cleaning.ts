...
# is now correctly captured instead of silently ending at the deal close`,
  ...
  {
    id: "qr-cleaning-20260926-dark-pool-volume",
    module: "cleaning",
    title: "Dark-pool prints inflating consolidated volume without moving price",
    difficulty: "core",
    question: `You notice a stock's daily volume spikes 3x on a day with no news and no unusual intraday price range or spread widening. Looking closer, the consolidated tape shows a single large print reported well after it actually executed, tagged with an off-exchange sale condition. What's going on, and why does it matter for a liquidity feature built from raw consolidated volume?`,
    thinking: `The consolidated tape is not "one exchange's order book," it's a legally mandated aggregation of every venue that's allowed to report trades in that name -- lit exchanges, but also alternative trading systems (dark pools) and broker-dealer internalization, plus negotiated block trades crossed away from the public book entirely. A large block negotiated off-exchange between two institutions prints to the tape (often with a reporting delay, and tagged with a sale-condition code marking it as such) without ever touching the visible order book, so it moves the day's volume total without moving the price and without reflecting any liquidity you could actually have traded against in real time. A feature that treats every consolidated share as equally "available" liquidity -- an ADV-based position cap, a participation-rate execution model, a naive volume-weighted signal -- silently overstates true tradable liquidity on days when a chunk of that volume was a privately negotiated block that happened to print through the public tape.`,
    answer: `The spike is a block trade or dark-pool print reported to the consolidated tape under an off-exchange sale condition -- real reported volume, but negotiated privately and not reflective of order-book liquidity, so it inflates the day's volume total without moving price or spreads. Filter or flag trades by their sale-condition code before treating consolidated volume as a liquidity proxy: an ADV-based capacity or position-sizing feature built from unfiltered consolidated volume overstates how much you could actually have traded against on the lit book that day.`,
