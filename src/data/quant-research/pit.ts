...
# "wrong" already sees the 03-01 signal on 03-01 itself`,
  ...
  {
    id: "qr-pit-20260916-universe-exclusion-lookahead",
    module: "pit",
    title: "Excluding a stock because it later went bankrupt is a lookahead bias in universe construction",
    difficulty: "hard",
    question: `A colleague builds their backtest universe by taking today's list of currently-listed S&P 500 constituents and pulling ten years of history for exactly those names, reasoning "I already handle survivorship bias by including delisted returns for names that dropped out." Is that enough? Where's the remaining bias?`,
    thinking: `Separate two different survivorship problems that get conflated. Including delisted RETURNS for names that were once in the index but later dropped out fixes one bias: you're not silently deleting their bad performance from the historical record. But this colleague's universe SELECTION rule -- start from today's constituent list and back-fill history -- has a second, subtler problem: it only includes names that survived (in some form) to today at all. A stock that entered the S&P 500 in 2016 and went fully bankrupt and was removed in 2019 never appears in today's constituent list, so it never enters this universe in the first place, at any date, even though on any day between 2016 and 2019 it was a perfectly real, tradeable, in-universe stock. You've used information from the FUTURE (whether the company still exists today) to decide whether to include it in the PAST universe -- that's lookahead bias in universe construction, distinct from and not fixed by handling delisted returns correctly for the names you did happen to include.`,
    answer: `Including delisted returns only fixes survivorship in the return series for names your universe selection happened to include -- it doesn't fix survivorship in the SELECTION itself. Building the universe by starting from today's constituent list and backfilling history silently drops any stock that was genuinely in the index at some past date but no longer exists in any form today (full bankruptcy, acquired then the acquirer also delisted, etc.), because membership in today's list is being used to decide inclusion in the past. The fix is point-in-time index membership: reconstruct which stocks were actually constituents on each historical date, independent of what exists today.`,
