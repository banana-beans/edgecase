...
print(round(dm_stat, 3), round(p_value, 4))`,
  ...
  {
    id: "qr-stats-20260914-jobson-korkie-correlated-sharpe",
    module: "stats",
    title: "Comparing two correlated strategies' Sharpe ratios: the Jobson-Korkie/Memmel test",
    difficulty: "hard",
    question: `Strategy A has a backtested Sharpe of 1.8, Strategy B has 1.3, both estimated on the same 5 years of daily returns. The two return series are correlated at about 0.6, since they trade overlapping instruments. Can you use a plain two-sample t-test (or Welch's) to say A's Sharpe is significantly higher than B's, and if not, what should you use instead?`,
    thinking: `A standard two-sample t-test assumes the two samples are INDEPENDENT, which is exactly the assumption that fails here -- A and B's daily returns are drawn on the same calendar days from overlapping markets, so they're correlated, and ignoring that correlation in a naive test gets the variance of the Sharpe DIFFERENCE wrong. What you actually want is a test built around Var(Sharpe_A minus Sharpe_B) that explicitly includes a covariance term between the two Sharpe estimates -- that's the Jobson-Korkie test (with Memmel's later correction to the variance formula, which is the version people actually use), which derives the asymptotic variance of the Sharpe difference as a function of both strategies' variances, their covariance, and their individual Sharpe ratios, then forms a z-statistic from that. The intuition worth holding onto: when two strategies are positively correlated, their Sharpe DIFFERENCE is often more precisely estimable than treating them as independent would suggest, because a lot of common market-wide noise cancels out of the comparison.`,
    answer: `No -- a plain or Welch's t-test assumes independence, which two correlated strategies violate, and gets the variance of the Sharpe difference wrong. Use the Jobson-Korkie test with Memmel's correction: it derives Var(Sharpe_A minus Sharpe_B) from both strategies' variances, their COVARIANCE, and their Sharpe ratios, then forms a z-statistic from the observed Sharpe gap divided by the square root of that variance. Positive correlation between the two strategies generally tightens the test relative to wrongly assuming independence.`,
...
print(round(se_annual_naive, 4), round(se_annual_lo, 4))`,
  ...
  {
    id: "qr-stats-20260919-window-search-overfit",
    module: "stats",
    title: "Multiple testing over backtest date ranges, not just signals",
    difficulty: "hard",
    question: `A researcher reports a fantastic backtest but mentions in passing that they "tried a few different start and end dates before settling on 2015-2023 because it looked cleanest." Why is that sentence more concerning than a mediocre single-window Sharpe ratio, and how do you think about correcting for it?`,
    thinking: `Recognize this as the multiple-testing problem wearing a different costume -- it's not about testing many signals, it's about testing many WINDOWS of the same signal, and the researcher already told you the selection was outcome-dependent ("looked cleanest" means the result was seen before the window was picked). Every candidate start/end date pair is effectively another trial, and picking the best-looking one after the fact inflates the apparent Sharpe exactly the way trying 100 random signals and reporting only the best one does -- except here there's no clean count of how many windows were tried the way there might be a clean count of signals, which makes a Bonferroni-style correction hard to apply mechanically. The practical response is less about a formula and more about discipline: distrust any backtest whose date range was chosen after seeing performance, re-run the identical strategy on windows the researcher did NOT choose (periods before and after the reported window, other market regimes), and treat "looks cleanest" as a direct admission that the reported number is upward biased by an unknown, unquantifiable amount.`,
    answer: `This is multiple-testing bias applied to date ranges instead of signals: trying several windows and keeping the best-looking one inflates the reported Sharpe the same way trying many signals and reporting only the winner does, except the "number of trials" here is informal and can't be cleanly Bonferroni-corrected. The fix is procedural, not statistical: insist the window be chosen before seeing performance, or fixed by an external rule like using all available history, and independently re-test on periods the researcher did not get to pick, including ones before and after the reported window.`,
...
# well above the nominal 5% -- Fisher's method is over-confident under correlation`,
  ...
  {
    id: "qr-stats-20260926-pbo",
    module: "stats",
    title: "Probability of Backtest Overfitting (PBO)",
    difficulty: "hard",
    question: `You searched over 200 parameter combinations for a strategy and are about to ship the one with the best in-sample Sharpe. Your manager asks: how do you actually know that specific configuration will still be the best one out-of-sample, rather than just the luckiest of the 200? What is the Probability of Backtest Overfitting, and how would you estimate it?`,
    thinking: `Notice this is a different question from the multiple-testing corrections you'd reach for first. Bonferroni or Benjamini-Hochberg attack the significance of a single test statistic across many candidates -- they ask "is this one number surprising given how many chances it had to look good." PBO asks something more direct and more relevant to what you're actually about to do: does the SPECIFIC configuration your selection procedure crowned as the in-sample winner tend to keep winning out-of-sample, or does it tend to fall to the bottom half once you check. The standard estimator, combinatorially symmetric cross-validation, makes this concrete: split the full track record of all 200 configurations into some number of contiguous time blocks, enumerate every way of partitioning those blocks into one half in-sample and its complementary half out-of-sample, and for each such partition find whichever configuration ranks best in-sample, then check where that SAME configuration lands in the out-of-sample ranking. PBO is simply the fraction of all those partitions where the in-sample winner ends up below the out-of-sample median. A PBO near 50 percent means your selection procedure is barely better than picking a random configuration from the 200; a PBO near 0 means the in-sample ranking genuinely carries information about future performance.`,
    answer: `PBO is the probability that whatever configuration your search procedure selects as best in-sample turns out to rank below the median out-of-sample. Estimate it with combinatorially symmetric cross-validation: repeatedly split the full history into every possible complementary pair of in-sample and out-of-sample halves, find the in-sample winner each time, and measure how often it lands out-of-sample below the median of all candidates. A PBO near 50 percent says your search procedure is essentially selecting noise, not skill; a PBO near 0 says the in-sample ranking is trustworthy.`,
