...
print(round(net_sharpe_at_aum(500e6, weight), 2))   # 10x AUM: cost eats into net edge`,
  ...
  {
    id: "qr-analytics-20260914-combined-sharpe-not-additive",
    module: "analytics",
    title: "Why two strategies' Sharpe ratios don't average (or add) to give the combined book's Sharpe",
    difficulty: "warmup",
    question: `Strategy A runs at a Sharpe of 1.5, Strategy B at a Sharpe of 1.0, each at equal capital. A PM asks what Sharpe to expect running both together. Is it just the average of 1.5 and 1.0, or somewhere close to it?`,
    thinking: `Sharpe is a ratio of mean to standard deviation, and standard deviations of a SUM of two return streams don't add linearly except in the special case of perfect correlation -- combined variance is Var(A) plus Var(B) plus twice the covariance of A and B, so combined volatility grows slower than a simple average whenever the two streams aren't perfectly correlated, because some of each one's risk is diversified away by the other. Combined mean return, in contrast, DOES average linearly regardless of correlation, since expectation is always additive. So the combined Sharpe is the average mean divided by a less-than-proportionally-scaled combined volatility -- meaning the combined Sharpe is generally HIGHER than the simple average of the two individual Sharpes whenever correlation is below one, and the lower the correlation, the bigger that diversification bonus. This is the entire economic case for running uncorrelated strategies together rather than picking only the single best one.`,
    answer: `No -- because volatility diversifies with correlation below one while expected return simply averages, the combined Sharpe is typically HIGHER than the average of the two individual Sharpes, not equal to it. The exact number depends on the correlation between the two streams: the lower the correlation, the bigger the boost above a naive average, which is precisely why running several only-decent, low-correlation strategies together can beat running one great one alone.`,
